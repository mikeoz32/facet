...
- `SemanticDb` with revision-safe node references, canonical semantic types,
  ...
  inference, overload lookup, receiver-generic and method-level `forall`
  substitution, call-site parameter specialization, scoped constant binding
  and inference, enum-member typing, branch-sensitive truthiness/`is_a?`
  narrowing, short-circuit flow, conditional binding merges, explicit-return
  collection, and strict/tolerant snapshots. Constants
  resolve across lexical scopes, methods, inheritance, includes, absolute
  paths, and `forall` metaclass paths. Undefined constants, constant cycles,
  constant-local leakage, and constants used as types have coded diagnostics;
  unknown facts suppress unsafe diagnostics.
...
- A committed Crystal 1.21 macro contract corpus. Facet matches exact expansion,
  ...
  semantic suite and matches all 2,736/2,736 distinct flag- and
  type-context-sensitive macro expansions. The gate includes all 1,077
  macro-call events and all 1,659 inline expansions, compares literal payloads
  as well as semantic AST shape, and removes no unsupported case.
  See [macro parity](MACRO_PARITY.md).

The semantic parity corpus captures 582 contracts from 449 official Crystal
1.21 semantic examples across nine suites. Facet currently supports 321
exactly and keeps all 261 remaining contracts in an explicit deferred manifest.
Every captured type, error, and no-error contract is classified. See
...
These query and semantic caches are process-local today: restarting a consumer
rebuilds them from source. A future persistent cache must be content-addressed
and versioned by the cache schema, Facet/Crystal versions, target options, and
compiler-context fingerprint. Runtime `FileId`, `NodeId`, and `TypeId` values
must be rebound when loading it rather than serialized as stable identities.
...
non-output control expressions produce an explicit expansion diagnostic. The
complete 3,288-example semantic suite additionally produces 2,736 unique
expansion contexts after exact-context deduplication; Facet matches all
2,736/2,736, with every captured event retained in the committed gate.
...
| Semantic contracts, first slice | 449 examples; 582 type/error/no-error contracts | 321 exact supported contracts; all 261 deferred contracts classified by reason |
| Facet native suite | — | 12,116 examples passing; all 4,378 upstream parser inputs committed locally; all 3,437 accepted trees pass both the recursive native contract and semantic projection oracle |
